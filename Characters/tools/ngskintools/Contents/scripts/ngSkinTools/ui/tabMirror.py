...
class InfluencesMappingConfiguration(object):
    ...
    def configureMapper(self):
        ...
        log.info("getting manual overrides")
        ...
        mapper.distanceMatchRule.mirrorAxis = ['X','Y','Z'].index(mll.getMirrorAxis().upper())
    ...
    def updateSelectionsInfluenceMapping(self):
        log.info("updating selection influence mapping")
        mapper = self.configureMapper()
        mapper.calculate()
        log.info("mapping calculated: %r" % mapper.mapping)
